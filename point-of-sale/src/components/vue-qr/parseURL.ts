// source: https://github.com/solana-labs/solana-pay/blob/master/core/src/parseURL.ts
// import { PublicKey } from '@solana/web3.js'
// import { Address } from '@polkadot/util-crypto/address'
import { decodeAddress } from '@polkadot/util-crypto'
import BigNumber from 'bignumber.js'
// import { HTTPS_PROTOCOL, SOLANA_PROTOCOL } from './constants'
import { HTTPS_PROTOCOL, ADDRESS_PREFIX } from './constants'
import { Address, Amount, Label, Link, Memo, Message, Recipient, Reference, SPLToken } from './types'
...
export function parseURL(url: string | URL): TransactionRequestURL | TransferRequestURL {
    ...
    if (url.protocol !== `${ADDRESS_PREFIX}:`) throw new ParseURLError('protocol invalid');
...
function parseTransferRequestURL({ pathname, searchParams }: URL): TransferRequestURL {
  ...
  if (splTokenParam != null) {
    try {
      // splToken = new Address(splTokenParam);
      splToken = decodeAddress(splTokenParam);
    } catch (error) {
  ...
  if (referenceParams.length) {
    try {
      ...
      reference = referenceParams.map((reference) => decodeAddress(reference));
    } catch (error) {
